...
void imprimir_dados_locacao(Locacao locacao, FILE *arquivo_filme){
  ...
  while(1){
    ...
    for(int i = 0; i < locacao.qtd_locados; i++){
      if(filmes.codigo_filme == locacao.filmes_locados[qtd]){
        printf("\t\tCodigo do filme: %d\n", locacao.filmes_locados[qtd]);
...
// Excluir uma locacao cadastrada //
...
// Consultar locação pelo seu código //
...
void consulta_nome_filme(FILE *arquivo_locacao, FILE *arquivo_filme){
  ...
  while(1){
    ...
    if(feof(arquivo_filme))
      break;
    if(strstr(nome_filme, filmes.titulo)){
      tinha_filme = 1;
      break;
    } 
  }
  ...
  if(tinha_filme == 1){
    while(1){
      ...
      for(int i = 0; i < locacao.qtd_locados; i++){
        if(locacao.filmes_locados[i] == filmes.codigo_filme){
          tinha_locacao = 1;
          break;
        }
      }
  ...
  if(tinha_filme == 1 && tinha_locacao == 1)
    imprimir_dados_locacao(locacao, arquivo_filme);   
  else
...
// Consulta na locacao por nome de usuario //
...
// Edita dados especificos da locacao //
